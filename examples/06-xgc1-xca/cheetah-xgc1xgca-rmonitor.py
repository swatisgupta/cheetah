...
class XGC1XCA(Campaign):
    ...
    supported_machines = ['local', 'summit']
    ...
    app_config_scripts = {'local':'setup.sh', 'summit':'setup.sh'}

    # Setup the sweep parameters for a Sweep
    sweep1_parameters = [
            # ParamRunner 'nprocs' specifies the no. of ranks to be spawned 
            p.ParamRunner       ('xgc1', 'nprocs', [8]),
            p.ParamEnvVar       ('xgc1', 'openmp', 'OMP_NUM_THREADS', [1]),
            p.ParamEnvVar       ('xgc1', 'savanna_model', 'SAVANNA_MONITOR_MODEL', ['outsetps2']),
            p.ParamEnvVar       ('xgc1', 'savana_params', 'SAVANNA_MONITOR_MPARAMS', ["0, 2, 5, 5, .bp"]),
            ...
            p.ParamKeyValue       ('xgc1', 'total_steps', 'XGC1_exec/input', 'sml_mstep', [10]),
            ...
            p.ParamRunner       ('xgca', 'nprocs', [8]),
            p.ParamEnvVar       ('xgca', 'openmp', 'OMP_NUM_THREADS', [1]),
            p.ParamEnvVar       ('xgca', 'savanna_model', 'SAVANNA_MONITOR_MODEL', ['outsetps2']),
            p.ParamEnvVar       ('xgca', 'savana_params', 'SAVANNA_MONITOR_MPARAMS', ["0, 2, 100, 5, .bp"]),
            ...
            p.ParamKeyValue       ('xgca', 'total_steps', 'XGCa_exec/input', 'sml_mstep', [10]),
    ...
    # Create the node-layout to run on local
    # Place the xgc1 and the analysis codes on separate nodes
    ...
    # of nodes. For example, for the xgc1, we have 32 ranks on one node and 512 nprocs, so Cheetah
    # will create 16 nodes of type 'sim_node'
    xgc1_node = SummitNode()
    xgca_node = SummitNode()
    for i in range(8):
        xgc1_node.cpu[i] = "xgc1:{}".format(i)
    xgc1_node.cpu[8] = "rmonitor:{}".format(0)
    for i in range(8):
        xgca_node.cpu[i] = "xgca:{}".format(i)
    seperate_node_layout = [xgc1_node, xgca_node]

    sweep1 = p.Sweep (parameters = sweep1_parameters, node_layout={'summit':seperate_node_layout}, rc_dependency={'xgca':'xgc1', 'restart1':'xgca'})
    sweep2 = p.Sweep (parameters = sweep1_parameters, node_layout={'local': [{'xgca': 8}, {'xgc1':8}, {'rmonitor':1}, {'restart1':1}]}, rc_dependency={'xgca':'xgc1','restart1':'xgca'})
    ...
    sweepGroup1 = p.SweepGroup ("sg-1", # A unique name for the SweepGroup
                                walltime=15060,  # Total runtime for the SweepGroup
                                ...
                                component_inputs = {'xgc1': ['run-xgc1.sh','XGC1_exec/adios.in','XGC1_exec/mon_in', 'XGC1_exec/petsc.rc',  SymLink('/homes/ssinghal/XGC-Devel-xgc1-xgca-coupling/xgc_build/xgc-es'), SymLink('XGC-1_inputs'), 'XGC1_exec/adioscfg.xml'], 
                                                    'xgca': ['run-xgca.sh','XGCa_exec/adios.in','XGCa_exec/mon_in', 'XGCa_exec/petsc.rc',  SymLink('/homes/ssinghal/XGC-Devel-xgc1-xgca-coupling/xgc_build/xgca'), SymLink('XGC-1_inputs'), 'XGCa_exec/adioscfg.xml'],
                                                    'rmonitor': ['/lustre/ssinghal/Dynamic_workflow_management/rmonitor/bin/r_monitor'] } #inputs required by codes
